 ...
 */
export function addAnyToFunctionParams( tsAstProject: Project ): Project {
	...
	addAny( constructorMinArgsMap );
	addAny( functionsMinArgsMap );
	addPublicPrivate( functionsMinArgsMap );
 ...
 */
function addAny( minArgsMap: Map<FunctionTransformTarget, number> ) {
...
function addPublicPrivate(minArgsMap: Map<FunctionTransformTarget, number>) {
	const fns = minArgsMap.keys();
	for (const fn of fns) {
		if (fn instanceof MethodDeclaration) {
			fn.setScope(Scope.Public)
		}
	}
}